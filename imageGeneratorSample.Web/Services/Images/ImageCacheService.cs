...
namespace imageGeneratorSample.Web.Services.Images;
...
public class ImageCacheService : IImageCacheService, IDisposable
{
    ...
    public async Task<string> CacheImageAsync(ReadOnlyMemory<byte> imageBytes, string contentType, string? fileName = null)
    {
        using var memoryStream = MemoryMarshal.TryGetArray(imageBytes, out var array) ?
                    new MemoryStream(array.Array!, array.Offset, array.Count) :
                    new MemoryStream(imageBytes.ToArray());
